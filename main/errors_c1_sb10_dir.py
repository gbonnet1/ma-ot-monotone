...
def u_func(x):
    return np.where(lp.dot_VV(x, x) < 1 / 4, 0, (np.sqrt(lp.dot_VV(x, x)) - 1 / 2) ** 2)


def du_func(x):
    return np.where(lp.dot_VV(x, x) < 1 / 4, 0, (2 - 1 / np.sqrt(lp.dot_VV(x, x))) * x)


def f_func(x):
    return np.where(lp.dot_VV(x, x) < 1 / 4, 0, 4 - 2 / np.sqrt(lp.dot_VV(x, x)))
