...
def f_func(x):
    ...
    x1 = np.stack(
        [
            np.cos(theta1) * x[0] + np.sin(theta1) * x[1],
            -np.sin(theta1) * x[0] + np.cos(theta1) * x[1],
        ]
    )
    x2 = np.stack(
        [
            np.cos(theta2) * x[0] + np.sin(theta2) * x[1],
            -np.sin(theta2) * x[0] + np.cos(theta2) * x[1],
        ]
    )
    x3 = np.stack(
        [
            np.cos(theta3) * x[0] + np.sin(theta3) * x[1],
            -np.sin(theta3) * x[0] + np.cos(theta3) * x[1],
        ]
    )
    t1 = 0.6
    t2 = 0.7
    t3 = 0.6
    return (
        np.where(x1[0] < t1, 0, 1)
        + np.where(x2[0] < t2, 0, 1)
        + np.where(x3[0] < t3, 0, 1)
    ) / (
        ma.DiskQuad(lambda x: 1, t1)
        + ma.DiskQuad(lambda x: 1, t2)
        + ma.DiskQuad(lambda x: 1, t3)
    )
